...
{
	int rotorLargestSetting = enigma->getLargestRotorSetting();
	int rank, size;
	int found = 0;
	...
	MPI_Bcast(&expectedLength, 1, MPI_INT, MPI_ROOT_PROCESS_RANK, MPI_COMM_WORLD); // expected lenght
	MPI_Bcast(&messageLength, 1, MPI_INT, MPI_ROOT_PROCESS_RANK, MPI_COMM_WORLD);

	if (rank != MPI_ROOT_PROCESS_RANK)
	{
		expectedMsg = new uint[expectedLength];
		messageToDecode = new uint[messageLength];
	}
	
	MPI_Bcast(expectedMsg, expectedLength, MPI_UNSIGNED, MPI_ROOT_PROCESS_RANK, MPI_COMM_WORLD); // expected msg
	MPI_Bcast(messageToDecode, messageLength, MPI_UNSIGNED, MPI_ROOT_PROCESS_RANK, MPI_COMM_WORLD);

	comparator->setExpectedFragment(expectedMsg, expectedLength);
	...
	for (int i = 0; i < size; i++)
	{
		...
		{
			startRotorPos = start;
			endRotorPos = end + chunks[i] + (rank == size - 1 ? 1 : 0);
			break;
		}
		start += chunks[i];
		end += chunks[i];
	...
	int *rotorsPositionMessage = new int[rotors];
	if (rank == MPI_ROOT_PROCESS_RANK)
	{
		MPI_Irecv(rotorsPositionMessage, rotors, MPI_INT, MPI_ANY_SOURCE, 0, MPI_COMM_WORLD, &request);
	}
	else
	{
		MPI_Irecv(&found, 1, MPI_INT, MPI_ANY_SOURCE, 0, MPI_COMM_WORLD, &all_recv_stop);
	}

	uint *r = new uint[MAX_ROTORS];
	int flag1 = 0;
	int flag2 = 0;
	for (r[0] = startRotorPos; r[0] <= endRotorPos; r[0]++)
										...
										for (r[9] = 0; r[9] <= rMax[9]; r[9]++)
										{
											...
											else
											{
												...
												if (solutionFound(r) || flag2)
												{
													...
													{
														MPI_Isend(&found, 1, MPI_INT, i, 0, MPI_COMM_WORLD, &all_recv_stop);
													}
													goto EXIT_ALL_LOOPS;
												}
											}
...
bool MPIEnigmaBreaker::solutionFound(uint *rotorSettingsProposal)
{
	...
	if (result && rank != MPI_ROOT_PROCESS_RANK)
	{
		...
		MPI_Send(messageByRotors, rotors, MPI_INT, MPI_ROOT_PROCESS_RANK, 0, MPI_COMM_WORLD);
...
void MPIEnigmaBreaker::setSampleToFind(uint *expected, uint expectedLengthArg)
{
	expectedLength = expectedLengthArg;
	expectedMsg = new uint[expectedLength];

	for (int i = 0; i < expectedLength; i++)
	{
		expectedMsg[i] = expected[i];
	}
